...
async function openDetail(id) {
  try {
    ...
    document.getElementById("detailHeader").innerHTML = `
      ...
      <div style="margin-top:8px">${acBadge(b.status)} &nbsp; 🎟️ #${b.tokenNumber || "-"}</div>
    `;
        ...
        tc.innerHTML = `
          ...
          ${partner ? `
            <div style="margin-top:8px;padding:10px;background:var(--green-100);border-radius:10px;color:var(--green-900);">
              <div>🚚 Vehicle: <b>${partner.vehicleNumber}</b></div>
              <div>🧑‍✈️ Driver: <b>${partner.driverName}</b></div>
              <div>📞 Phone: <b>${partner.driverPhone}</b></div>
            </div>
          ` : `
    ...
    if (b.qualityCheck && b.qualityCheck.grade) {
      ...
      qc.innerHTML = `<h3>🔍 Quality Check</h3>
    ...
    document.getElementById("notifLog").innerHTML = (b.notificationsLog || [])
      .slice().reverse().map(n => `<div style="margin-bottom:6px">📩 ${n.message}</div>`).join("") || "-";
